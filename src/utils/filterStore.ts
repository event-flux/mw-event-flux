...
const storeBuilders = {
  Item(
    this: IExtendStoreBase,
    StoreClass: IExtendStoreBaseConstructor,
    storeKey: string,
    stateKey: string,
    options: any
  ): void {
  ...
  List(
    ...
    let storeObserver = (store: IExtendStoreBase, index: number) => {
      return store.observe(state => {
        ...
        this.setState({
          [stateKey]: [...oldStates.slice(0, index), state, ...oldStates.slice(index + 1)],
        });
      });
    };
    ...
    if (this.setStore) {
      return this.setStore(storeKey, newStore);
    }
...
const storeObservers = {
  Item(this: IExtendStoreBase, storeKey: string, stateKey: string, options: any) {
    let store: IExtendStoreBase = this.getStore
      ? (this.getStore(storeKey) as IExtendStoreBase)
      : ((this as any)[storeKey] as IExtendStoreBase);
  ...
  List(this: IExtendStoreBase, storeKey: string, stateKey: string, options: any) {
  ...
  Map(this: IExtendStoreBase, storeKey: string, stateKey: string, options: any) {
...
export function filterOneStore(
  StoreClass: IExtendStoreBaseConstructor,
  filterOptions?: FilterStoreOptions
): IStoreFilters | null {
  if (!StoreClass) {
    return null;
  }
    ...
    if (isFunction(declarer)) {
      ...
      if (StoreDeclarer.isStore(declarer)) {
        ...
        subStoreInfos.push([
          ...
          extendStore,
          storeName,
          nowOptions && nowOptions.directInsert ? null : stateKey,
          nowOptions,
        ]);
      }
    }
  ...
  StoreClass.prototype.buildStores = function() {
    subStoreInfos.forEach(([type, ThisStoreClass, storeKey, stateKey, options]) => {
      storeBuilders[type].call(this, ThisStoreClass, storeKey, stateKey!, options);
  ...
  StoreClass.prototype.startObserve = function() {
    subStoreInfos.forEach(([type, _, storeKey, stateKey, options]) => {
      let store = this.getStore ? this.getStore(storeKey) : this[storeKey];
