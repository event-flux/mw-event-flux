...
export default class ElectronMainClient implements IMainClient {
  ...
  // The main process will invoke handleRendererMessage to
  // handle the message and send the result back to renderer process
  private handleRendererDispatch = (event: Event, clientId: string, invokeId: string, stringifiedAction: string) => {
    ...
    this.mainClientCallbacks.handleRendererMessage(stringifiedAction).then(
      result => {
